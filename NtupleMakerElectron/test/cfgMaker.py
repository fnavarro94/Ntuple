...
for j in range(1,33):
	fBash = open(outputFile +str(int(sys.argv[2])+5*(j-1))+"-"+str(int(sys.argv[3])*j)+".sh", 'w')


        print >>chainRun, './'+outputFile +str(int(sys.argv[2])+5*(j-1))+"-"+str(int(sys.argv[3])*j)+".sh" ' &' 
	...
	for i in range(int(sys.argv[2])+5*(j-1), int(sys.argv[3])*j):
		print i
